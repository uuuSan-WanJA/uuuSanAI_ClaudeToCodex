...
| Project-scoped Codex config | `.codex/config.toml` | .codex/config.toml with developer_instructions that reinforce delegation-first routing and preserved owner boundaries | rewrite | planned | Detected project-scoped Codex config in `.codex/config.toml`. |
| Workflow parity closure | `compatibility-matrix.md`, `docs/pipeline.md` | Codex-side artifacts and verification that preserve command, hook, wrapper, and launcher behavior end-to-end | rewrite | planned | Detected workflow-parity language in `compatibility-matrix.md`, `docs/pipeline.md`. |
| Subagent architecture preservation | `AGENTS.md`, `CLAUDE.md`, `compatibility-matrix.md`, `README.md`, `docs/pipeline.md` | Codex-facing docs and workflows that preserve the source Claude subagent structure and owner boundaries | rewrite | planned | Detected structure-preservation language in `AGENTS.md`, `CLAUDE.md`, `compatibility-matrix.md`, `README.md`, `docs/pipeline.md`. |
| Runtime contract installation gate | `compatibility-matrix.md` | Target remains blocked or incomplete until the binding runtime contract is installed in repo-root guides | rewrite | planned | Detected completion-gate language in `compatibility-matrix.md`. |
...
| Wrapper commands | `scripts/patch-harnessmaker-delegation-first.ps1`, `scripts/patch-harnessmaker-owner-routing.ps1`, `scripts/patch-woore-fsf2-codex.ps1`, `scripts/patch-woore-fsf2-workflow-parity.ps1` | Shell recipes and helper scripts that preserve behavior across runtimes | rewrite | planned | Detected 4 wrapper script(s) with direct Claude references. |
...
| Operating system assumptions | posix, windows | Codex-compatible platform policy or dual-path wrappers | rewrite | planned | Evidence: `AGENTS.md`, `CLAUDE.md`, `scripts/patch-harnessmaker-delegation-first.ps1`, `scripts/patch-harnessmaker-owner-routing.ps1`, `scripts/patch-woore-fsf2-codex.ps1`, `scripts/patch-woore-fsf2-workflow-parity.ps1` |
| Shell assumptions | bash, powershell | Codex shell usage and script normalization rules | rewrite | planned | Evidence: `AGENTS.md`, `CLAUDE.md`, `scripts/patch-harnessmaker-delegation-first.ps1`, `scripts/patch-harnessmaker-owner-routing.ps1`, `scripts/patch-woore-fsf2-codex.ps1`, `scripts/patch-woore-fsf2-workflow-parity.ps1` |
